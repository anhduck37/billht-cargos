...
class AutoMapEmsNewAddressCommand extends Command
{
    protected $signature = 'address:auto-map-ems
        {--limit=0 : Gioi han so xa/phuong cu can quet, 0 la tat ca}
        {--province-code= : Chi quet mot tinh/thanh cu theo ma API tra cuu dia chi, vi du Can Tho = 92}
        {--dry-run : Chi kiem tra, khong ghi DB}
        {--only-missing=1 : Chi bo sung mapping EMS dang thieu}
        {--write-ambiguous=0 : Cho phep ghi ca mapping co nhieu ung vien ngang diem}';

    protected $description = 'Auto create EMS mappings for new 2025 wards by converting old wards to new wards';
    ...
    public function handle(Address2025Service $addressService)
    {
        ...
        $this->oldEmsIndex = $this->buildOldEmsIndex();

        if (empty($this->oldEmsIndex)) {
            $this->error('Chua co danh muc EMS cu trong bang citys/districts/wards. Hay chay address_code_ems truoc.');
            return 1;
        }
        ...
        if (!empty($result['review'])) {
            $path = storage_path('logs/ems_new_address_mapping_review.json');
    ...
    {
        $oldEms = $this->findOldEmsAddress($province['name'] ?? '', $district['name'] ?? '', $ward['name'] ?? '');
        if (!$oldEms) {
            return;
        }
        ...
        foreach ($newAddresses as $newAddress) {
            ...
            $this->newCandidates[$newWard->id][] = [
                ...
                'partner_province_code' => $oldEms['province_code'],
                'partner_district_code' => $oldEms['district_code'],
                'partner_ward_code' => $oldEms['ward_code'],
    ...
    {
        $note = 'Auto mapped EMS from tracuudiachi old->new API';
    ...
    private function buildOldEmsIndex()
    {
        $index = [];
        ...
        Ward::chunkById(1000, function ($wards) use (&$index, $cities, $districts) {
            foreach ($wards as $ward) {
                ...
                if (!$district || empty($district->ems_code) || empty($ward->ems_code)) {
                    continue;
                }

                $city = $cities->get($district->city_id);
                if (!$city || empty($city->ems_code)) {
                    continue;
                }

                $key = $this->normalize($city->city_name) . '|' . $this->normalize($district->district_name) . '|' . $this->normalize($ward->ward_name);
                $index[$key] = [
                    'province_code' => $city->ems_code,
                    'district_code' => $district->ems_code,
                    'ward_code' => $ward->ems_code,
                ];
            }
        });
    ...
    private function findOldEmsAddress($provinceName, $districtName, $wardName)
    {
        $key = $this->normalize($provinceName) . '|' . $this->normalize($districtName) . '|' . $this->normalize($wardName);
        if (isset($this->oldEmsIndex[$key])) {
            return $this->oldEmsIndex[$key];
        }
        ...
        foreach ($this->oldEmsIndex as $emsKey => $emsValue) {
            list($emsProvince, $emsDistrict, $emsWard) = explode('|', $emsKey);
            if ($emsProvince !== $provinceKey || $emsDistrict !== $districtKey) {
                continue;
            }

            if ($emsWard === $wardKey || strpos($emsWard, $wardKey) !== false || strpos($wardKey, $emsWard) !== false) {
                return $emsValue;
            }
        }
    ...
    {
        $value = str_replace('Đ', 'D', (string)$value);
