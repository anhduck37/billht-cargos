    ...
    <div class="col-sm-6">
        <div class="card">
            <div class="card-body p-4">
                <h3 class="card-title text-primary"><i class="fas fa-user-tie mr-2"></i>Thông tin người gửi</h3>
                ...
                <div class="form-row">
                    <input type="hidden" name="sender[address_scheme]" value="new">
                    <div class="form-group col-md-6">
                        <label>Tỉnh / Thành phố mới <span style="color: red; font-weight: bold;">(*)</span></label>
                        <select @if(auth()->user()->level == \App\User::LEVEL_POSTMAN) disabled @endif name="sender[new_province_id]" id="sender_province" class="form-control">
                            <option value=""></option>
                            @foreach($newProvinces as $province)
                                <option value="{{$province->id}}" @if(isset($order->sender) && $order->sender->new_province_id == $province->id) selected @endif >{{$province->name}}</option>
                            @endforeach
                        </select>
                        @if ($errors->has('sender.new_province_id'))
                            <span class="invalid-feedback" style="display: block;" role="alert">
                                <strong style="color: red;">{!! $errors->first('sender.new_province_id') !!}</strong>
                            </span>
                        @endif
                    </div>
                    <div class="form-group col-md-6">
                        <label>Xã / Phường mới <span style="color: red; font-weight: bold;">(*)</span></label>
...
{{--                </div>--}}
    ...
    <div class="col-sm-6">
        <div class="card">
            <div class="card-body p-4">
                <h3 class="card-title text-success"><i class="fas fa-user-check mr-2"></i>Thông tin người nhận</h3>
                <!-- <p style="color: red;">(Lưu ý: Tất cả các trường được đánh dấu * là bắt buộc)</p> -->
                <div class="form-row">
                    <div class="form-group col-md-6">
                        <label>Tên cá nhân/ Công ty <span style="color: red; font-weight: bold;">(*)</span></label>
                ...
                <div class="form-row">
                    <input type="hidden" name="receiver[address_scheme]" value="new">
                    <div class="form-group col-md-6">
                        <label>Tỉnh / Thành phố mới <span style="color: red; font-weight: bold;">(*)</span></label>
                        <select @if(auth()->user()->level == \App\User::LEVEL_POSTMAN) disabled @endif name="receiver[new_province_id]" id="receiver_province" class="form-control">
                            <option value=""></option>
                            @foreach($newProvinces as $province)
                                <option value="{{$province->id}}" @if(isset($order->receiver) && $order->receiver->new_province_id == $province->id) selected @endif>{{$province->name}}</option>
                            @endforeach
                        </select>
                        @if ($errors->has('receiver.new_province_id'))
                            <span class="invalid-feedback" style="display: block;" role="alert">
                                <strong style="color: red;">{!! $errors->first('receiver.new_province_id') !!}</strong>
                            </span>
                        @endif
                    </div>
                    <div class="form-group col-md-6">
...
<div class="card mt-4">
    <div class="card-body p-4">
        <h3 class="card-title text-info"><i class="fas fa-box-open mr-2"></i>Thông tin vận đơn</h3>
        <div class="form-group">
            <div class="form-row">
...
{{--                    <input @if(auth()->user()->level == \App\User::LEVEL_POSTMAN) disabled @endif type="text" class="form-control" name="order[order_date]" value="10/24/1984" id="order_date">--}}
                ...
                @endif
                @if(in_array(auth()->user()->level, [\App\User::LEVEL_USER, \App\User::LEVEL_ADMIN, \App\User::LEVEL_STAFF]))
                    <div class="@if(in_array(auth()->user()->level, [\App\User::LEVEL_USER, \App\User::LEVEL_ADMIN, \App\User::LEVEL_STAFF])) col-md-3 @else col-md-6 @endif mb-3">
    ...
    <div class="form-row">
        ...
        <div id="results" style="text-align: center">
            @if (isset($order->image))
                <img class="imageShow" style="{{$order->image->type_upload == \App\OrderImage::TYPE_IMAGE_WEBCAM  ? 'transform: rotate(270deg);': ''}}" src="{{$order->image->type_save == \App\OrderImage::SAVE_GOOGLE_DRIVE ? (config('google_drive.url').$order->image->file_id) : asset('uploads/'.$order->image->image)}}" />
                <div class="mt-2">
    ...
    function startScanner() {
        ...
        $(function() {
            @if(isset($update))
                ...
                let provinceUpdateSender = order && order.sender && order.sender.new_province_id;
                let wardUpdateSender = order && order.sender && order.sender.new_ward_id;
                if (provinceUpdateSender) {
                    renderSelectNew('sender_ward', provinceUpdateSender, 'new-ward', wardUpdateSender);
                }

                let provinceUpdateReceiver = order && order.receiver && order.receiver.new_province_id;
                let wardUpdateReceiver = order && order.receiver && order.receiver.new_ward_id;
                if (provinceUpdateReceiver) {
                    renderSelectNew('receiver_ward', provinceUpdateReceiver, 'new-ward', wardUpdateReceiver);
                }
            @endif

            $('#sender_province').on('change', function (e) {
                let province_id = e.target.value;
                $('#sender_ward').html('<option value=""></option>')
                if(province_id) renderSelectNew('sender_ward', province_id, 'new-ward');
            });

            $('#receiver_province').on('change', function (e) {
                let province_id = e.target.value;
                $('#receiver_ward').html('<option value=""></option>')
                if(province_id) renderSelectNew('receiver_ward', province_id, 'new-ward');
            });
            $(function() {
